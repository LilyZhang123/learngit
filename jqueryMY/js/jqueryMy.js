 ...
 * 
 * ¡X Fixed #34: styler no more corrupts @keyframes definitions
 * ¡X Fixed #35: $.my.locale(locale) works correctly during runtime
 *
...
;(function ($) {
  ...
  var MY = {
    ...
    msg:{
      ...
      formError:{en:"Form error",ru:"?????? ?????"},
      initFailed:{
        en:'<p class="my-error">Form init failed</p>',
        ru:'<p class="my-error">?????? ????????????? ?????</p>'
      },
  
      badInput:{en:"Invalid input", ru:"???????? ????????"},
      patternMismatch:{en:"Pattern mismatch", ru:"?? ????????????? ???????"},
      rangeOverflow:{en:"Over maximum", ru:"?????? ?????????"},
      rangeUnderflow:{en:"Under minimum", ru:"?????? ????????"},
      stepMismatch:{en:"Step mismatch", ru:"?? ?????? ????"},
      tooLong:{en:"Too long", ru:"??????? ??????"},
      typeMismatch:{en:"Invalid type", ru:"???????? ???"},
      valueMissing:{en:"Required", ru:"???????????? ????"}
    },
  ...
  var f = ({
    ...
    "css2json": function(css){
      var acc = "", res = (
        ...
        .replace(/@charset[^;]+;/gim,'')
        .replace(/[\n\t\s]+/g,' ')
        .replace(/\s*@media[^{]*\{/g,function(e){return e.to(-1).trim()+"?"})
        .replace(/\}/g,'}?')
        .replace(/^\n+/g,"").replace(/[\n\s]+$/g,"")
        .split('?')
        .compact(true)
  ...
  function _overlap (a,b) {
    ...
    // non-obj properties are replaced, obj ¡V merged plain
  ...
  function _localCache (A1, A2) {
    // ( no args ) ¡V returns all forms obj container
    // ({manifest},  {container}) ¡V caches form in container, id must be defined, return form or null
    // ({manifest}) ¡V caches form in local container, id must be defined
    // ("form.Id", "exist") ¡V true/false
    // ("form.Id", {container}) ¡V get from container
    // ("form.Id") ¡V get from local cache
    var ref, obj;
    if (isS(A1)) {
      ref=A1;
 ...
 *     1. jQuery image ¡V will raise modal with the image and text from title or data-text attributes
 *     2. HTML string ¡V will raise modal with html content
 *     3. Object of type
 *       {
 ...
 *         nose:"", left|right|top|bottom ¡V where to put nose
 ...
 *      opened and promise ¡V pending.
 *
 *     4. null, undefined or false ¡V close dialog and calls done(formErrors, data),
 *        if done return false or null promise is resoved with data,
 *        else modal stays open
 *     5. true (strict boolean) ¡V close dialog and calls done (null, null),
...
(function ($){
  ...
  $.fn.modal = function (obj0, done0, width0) {
    ...
    // close btn
    if (m.close) {
      $cl = $(isS(m.close)? m.close:'<div class="my-modal-close" title="Close">¡Ñ</div>')
